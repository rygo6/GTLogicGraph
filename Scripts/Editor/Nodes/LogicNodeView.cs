    ...
    public class LogicNodeView : Node
    {
        ...
        private void UpdatePortInputs()
        {
            foreach (var port in inputContainer.OfType<LogicPort>())
            {
                ...
                {
                    var portInputView = new PortInputView(port.Description) { style = { positionType = PositionType.Absolute } };
        ...
        private void UpdatePortInput(LogicPort port)
        {
            ...
            var inputView = _portInputContainer.OfType<PortInputView>().First(x => Equals(x.Description, port.Description));
